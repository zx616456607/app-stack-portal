...
function getColumns(self): Array<any> {
  ...
  const columns = [{
    ...
    key: 'name',
    render: (name) => {
      return <UnifiedLink to={`/workloads/Job/${name}`}>
      <Ellipsis title={name}>
      {name}
    </Ellipsis>
    </UnifiedLink>
    ...
    key: 'operation',
    render: (_, record) => {
      const query = { edit: true, type: 'Job', name: record.name }
      const dropdown = (
      ...
      return (
        ...
        <Dropdown.Button
          ...
          type="ghost"
          onClick={() =>
            unifiedHistory.push(`/workloads/Job/yamlEditor/createWorkLoad?${queryString.stringify(query)}`)}
        >
          查看/编辑Yaml
        </Dropdown.Button>
      </div>
